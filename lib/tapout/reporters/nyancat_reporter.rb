...
module Tapout::Reporters
  ...
  # Nyancat progress reporter.
  #
  class NyancatReporter < Abstract
    VERSION = "0.0.1"
    ...
    def start_suite(suite)
      @example_count = suite["count"]
      @current = @color_index = @passing_count = @error_count = @failure_count = @pending_count = 0
      @example_results = []
      @failed_examples = []
      @pending_examples = []
      $stdout.sync = true
    ...
    # @return nothing
    def dump_progress
      $stdout.print(progress_lines.join("\n") + eol)
    end
    ...
    # @return [Fixnum]
    def terminal_width
      default_width = `stty size <&2`.split.map { |x| x.to_i }.reverse.first - 1
